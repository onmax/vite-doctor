...
const total = computed(
  () =>
    props.result.summary.blocker +
    props.result.summary.error +
    props.result.summary.warn +
    props.result.summary.info,
);
...
const visibleFindings = computed(() => {
  ...
  return [...props.result.diagnostics]
    ...
    .map((d) => ({
      severity:
        d.severity === "blocker" || d.severity === "error"
          ? "high"
          : d.severity === "warn"
            ? "medium"
            : "low",
...
<template>
  <section class="overflow-hidden bg-white dark:bg-neutral-900">
    ...
    >
      <div class="flex flex-col items-center">
        <svg viewBox="0 0 130 80" class="w-36 sm:w-40" aria-hidden="true">
          ...
          <path
            ...
            d="M 15 65 A 50 50 0 0 1 115 65"
            fill="none"
            stroke="url(#gauge-gradient-live)"
            stroke-width="9"
            stroke-linecap="butt"
        ...
        <div class="-mt-7 flex items-baseline gap-1">
          <span
            class="text-4xl font-semibold tabular-nums tracking-tight text-neutral-900 dark:text-neutral-100"
            >{{ progress }}</span
          >
      ...
      >
        ...
        <p class="mt-1 text-sm text-neutral-500 tabular-nums dark:text-neutral-500">
          {{ framework === "nuxt" ? "Nuxt" : "Vue" }} Doctor &middot;
    ...
    <div class="border-t border-neutral-100 px-6 pt-4 sm:px-8 dark:border-neutral-800">
      <div
        class="hidden gap-4 pb-3 text-[0.6875rem] font-medium uppercase tracking-wider text-neutral-500 sm:grid sm:grid-cols-[100px_1fr_220px_auto] dark:text-neutral-500"
      >
        <span>Severity</span><span>Rule</span><span>Location</span
      ...
      <ul role="list">
        <li
          ...
          :key="`${f.location}-${i}`"
          class="flex flex-col gap-2 border-t border-neutral-100 py-4 sm:grid sm:grid-cols-[100px_minmax(0,1fr)_220px_auto] sm:items-start sm:gap-4 dark:border-neutral-800"
        >
          <span class="inline-flex items-center gap-2">
            <span
          ...
          <code
            class="truncate font-mono text-xs text-neutral-500 sm:self-center sm:text-[0.8125rem] dark:text-neutral-500"
            :title="f.location"
            >{{ f.location }}</code
          >
          <UIcon
            name="i-lucide-arrow-up-right"
            class="hidden size-4 self-center text-neutral-300 sm:inline-block dark:text-neutral-700"
            aria-hidden="true"
          />
        </li>
      </ul>
